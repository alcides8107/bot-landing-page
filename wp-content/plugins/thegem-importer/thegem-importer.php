...
function thegem_importer_get_purchase() {
	...
	$theme_options['purchase_code'] = 'purchase_code';
...
function thegem_importer_get_import_step_1() {
	...
	$response = wp_remote_get( WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $_REQUEST['import'] . '/import.json' );
	if(!is_wp_error($response)) {
		$rp_data = json_decode($response['body'], true);
		if(is_array($rp_data)) {
			ob_start();
			...
			$import_data['image'] = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $_REQUEST['import'] . '/screenshot.jpg';
			...
			foreach($sliders as $alias => $filename) {
				...
				$sliders[$alias] =  WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $_REQUEST['import'] . $filename;
			}
	...
	<form class="import-data-select <?php echo esc_attr($_REQUEST['import_type']); ?>">
		...
		<?php
			if($_REQUEST['import_type'] == 'part') {
				...
				$resp_content = wp_remote_get( WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $_REQUEST['import'] . '/content_singles.json' );
				...
				$resp_links = wp_remote_get( WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $_REQUEST['import'] . '/posts_links.json' );
...
function thegem_importer_get_import_step_3() {
	...
	if(is_array($import_data)) {
		...
		$content_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/content.xml';
		...
		$attachments_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/content_attachments.json';
		...
		$options_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/theme_options.json';
		...
		$page_settings_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/page_settings.json';
		...
		$menu_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/content_menu.xml';
		//$widgets_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/widgets.json');
		$widgets_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/widgets.json';
		...
		$forms_file_temp = WP_CONTENT_URL . '/uploads/thegem-wpbakery/' . $import_selection['import'] . '/content_contact_forms.json';
