...
function thegem_scripts() {
	...
	wp_register_style('icons-thegemdemo', get_template_directory_uri() . '/css/icons-thegemdemo.css');
	...
	wp_register_style('thegem-wpb-animations', get_template_directory_uri() . '/css/thegem-wpb-animations.css');
}
...
function thegem_admin_scripts_init() {
	...
	wp_enqueue_style('thegem-admin-styles', get_template_directory_uri() . '/css/thegem-admin.css', array(), '4.6.0');
	...
	wp_enqueue_script('thegem-admin-functions', get_template_directory_uri() . '/js/thegem-admin_functions.js', array('jquery'), '4.6.0');
...
function thegem_vc_frontend_editor_enqueue_js_css() {
	...
	wp_enqueue_style('icons-thegemdemo', get_template_directory_uri() . '/css/icons-thegemdemo.css');
		...
		if (is_singular() || is_tax() || is_category() || is_tag() || is_search() || is_archive() || is_post_type_archive('product') || is_tax('product_cat') || is_tax('product_tag') || is_404()) {
			...
			if($page_data['title_background_type']) {
				if($page_data['title_background_type'] == 'image') {
					...
					$video_bg = thegem_video_background($page_data['title_background_video_type'], $page_data['title_background_video'], $page_data['title_background_video_aspect_ratio'], $page_data['title_menu_on_video'], $page_data['title_background_video_overlay'], $page_data['title_video_overlay_opacity'], $page_data['title_background_video_poster']);
			...
			if(isset($page_data['title_show']) && !$page_data['title_show']) {
				$title_show = 0;
			}
		}
			...
			if($page_data['title_style'] == 2 && !empty($page_data['title_template']) && function_exists('vc_is_page_editable') && vc_is_page_editable()) {
				$buttons_html .= '<div class="edit-template-overlay">';
					$buttons_html .= '<div class="buttons">';
						...
						$buttons_html .= '" target="_blank">'.__('Edit Custom Title', 'thegem').'</a>';
						//$buttons_html .= '<a href="#" target="_blank" class="doc">?</a>';
					$buttons_html .= '</div>';
				$buttons_html .= '</div>';
			}
			return '<div id="page-title" class="page-title-block page-title-alignment-' . esc_attr($alignment) . ' page-title-style-' . esc_attr($title_style) . ' ' . esc_attr(implode(' ', $title_classes)) . ($parallax_bg ? ' page-title-parallax-background-wrap' : '') . '">
						'.$buttons_html.'
